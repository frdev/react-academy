...
export default function DashboardPage() {
  const { isLoaded } = useInitProgress()
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <header className="border-b border-gray-800 bg-gray-900/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-white">React Academy</h1>
            <p className="text-xs text-gray-500">30 dias de React avançado</p>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {completedDays === 0 && (
          <section>
            <div className="bg-blue-900/20 border border-blue-800 rounded-xl p-6 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-white mb-1">Bem-vindo ao React Academy!</h2>
                <p className="text-gray-400 text-sm">Comece sua jornada de 30 dias de React avançado.</p>
              </div>
              <button
                onClick={() => navigate('/day/day-01/theory')}
        ...
        {/* Day Grid */}
        <section>
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Os 30 Dias</h2>
          <DayGrid />
        </section>
      </main>
    </div>
